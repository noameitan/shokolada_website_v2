...
<section id="featured_products_sldier_block_center{$smarty.capture.column_slider}" class="block products_block {if !$column_slider} section {/if} {if $hide_mob} hidden-phone {/if}">
	<h4 class="title_block mar_b1"><span>{l s='Featured Products' mod='homefeaturedslider'}</span></h4>
    <script type="text/javascript">
    //<![CDATA[
    var featured_itemslider_options{$smarty.capture.column_slider};
    //]]>
    </script>
	{if isset($products) AND $products}
    <div id="featured-itemslider{$smarty.capture.column_slider}" class="flexslider">
    	{if $column_slider}
    ...
    {literal}
    jQuery(function($) {
        featured_itemslider_options{/literal}{$smarty.capture.column_slider}{literal} = {
    		...
    		controlNav: false,
    		controlsContainer: "#featured-itemslider{/literal}{$smarty.capture.column_slider}{literal} .nav_top_right",
    		itemWidth: 260,
        ...
        $('#featured-itemslider{/literal}{$smarty.capture.column_slider}{literal} .sliderwrap').flexslider(featured_itemslider_options{/literal}{$smarty.capture.column_slider}{literal});
        var featured_flexslider_rs{/literal}{$smarty.capture.column_slider}{literal};
        $(window).resize(function(){
            clearTimeout(featured_flexslider_rs{/literal}{$smarty.capture.column_slider}{literal});
            var rand_s = parseInt(Math.random()*200 + 300);
            featured_flexslider_rs{/literal}{$smarty.capture.column_slider}{literal} = setTimeout(function() {
                ...
                var flexslide_object = $('#featured-itemslider{/literal}{$smarty.capture.column_slider}{literal} .sliderwrap').data('flexslider');
