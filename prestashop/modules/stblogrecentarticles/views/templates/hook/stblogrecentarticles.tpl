...
<section id="st_blog_recent_article" class="block section">
    ...
    {if is_array($blogs) && $blogs|count}
    	...
    	{foreach $blogs as $blog}
    		<li class="block_blog {if $smarty.foreach.blog.first}first_item{elseif $smarty.foreach.blog.last}last_item{/if} clearfix">
                ...
                {elseif $blog.type==3}
                    <div class="blog_image"><a href="{$blog.link|escape:'html'}" rel="bookmark" title="{$blog.name|escape:'htmlall':'UTF-8'}"><img src="{$blog.cover.links.small}" alt="{$blog.name|escape:'htmlall':'UTF-8'}" width="{$imageSize[1]['small'][0]}" height="{$imageSize[1]['small'][1]}" class="hover_effect" /></a></div>
                {/if}
                <div>
    ...
    {else}
        <p class="warning">{l s='There are no recent posts' mod='stblogrecentarticles'}</p>
    {/if}
</section>
<!-- /St Blog recent articles  -->