...
class StEasyTabsClass extends ObjectModel
{
            ...
            if($category->id)
            {
                ...
                if(is_array($category_parents) && count($category_parents))
                    foreach($category_parents as $v)
                        $categories[] = $v['id_category'];
            }
        }
        $where = ' ret.`allitems`=1 ';
        if(count($categories))
            $where .= ' OR ret.`id_category` IN ('.implode(',',$categories).')';
        if($id_product)
    ...
    public static function delteByProductId($id_product)
    {
        if(!$id_product)
            return false;
        return Db::getInstance()->execute('
            ...
            WHERE `id_product` ='.(int)$id_product.Shop::addSqlRestrictionOnLang()
        );
    }
