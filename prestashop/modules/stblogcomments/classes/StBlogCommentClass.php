...
class StBlogCommentClass extends ObjectModel
{
    ...
    public static function countComments($id_blog,$id_shop=0,$active=true)
    {
        if (!$id_blog || !Validate::isUnsignedInt($id_blog) || !Validate::isUnsignedInt($id_shop))
            return 0;
            
        $where = '`id_st_blog` = '.$id_blog;
    ...
    public static function getCommentsRss($base_url, $limit = 15)
    {
        ...
        foreach($rs AS $v)
        {
            ...
            $ret[] = array(
                'title' => $v['name'],
                'desc'  => $desc,
                'link'  => Tools::safeOutput($this->context->link->getModuleLink('stblog','stblog',array('id_blog'=>$v['id_st_blog'],'rewrite'=>$v['link_rewrite']))),
    ...
    public static function blogAcceptComment($id_blog)
    {
        if (!$id_blog)
            return false;
        return Db::getInstance()->getValue('
        SELECT accept_comment FROM `'._DB_PREFIX_.'st_blog`
        WHERE id_st_blog = '.(int)$id_blog.'
        ') > 0;
    }
}
