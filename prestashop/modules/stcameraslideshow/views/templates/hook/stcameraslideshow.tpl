...
{if isset($slide_group)}
    ...
    {foreach $slide_group as $slide}
        {if isset($slide['slide'])}
            <div class="slideshow_block {if $slide['hide_on_mobile']}hidden-phone{/if} mar_b2 clearfix">
                    ...
                    {foreach $slide['slide'] as $banner}
                        <div data-thumb="{if (isset($banner.thumb_multi_lang) && $banner.thumb_multi_lang)}{$banner.thumb_multi_lang}{else}{$banner.thumb}{/if}" data-src="{if (isset($banner.image_multi_lang) && $banner.image_multi_lang)}{$banner.image_multi_lang}{else}{$banner.image}{/if}" {if $banner.url && !$banner.button} data-link="{$banner.url|escape:'html'}" data-target="{if $banner.new_window}_blank{else}_self{/if}" {/if}>
                            ...
                            <div class="camera_caption fadeFromBottom">
                                <div class="container">
                                    <div id="camera_caption_{$banner.id_st_camera_slideshow}" class="camera_caption_box hidden-phone {$banner.text_position} {if $banner.text_align eq 2}text-left{elseif $banner.text_align eq 3}text-right{else}text-center{/if}">
                                    {$banner.description}
                                    <div class="clearit"></div>
                                    {if $banner.url && $banner.button}
                                        <a href="{$banner.url}" target="{if $banner.new_window}_blank{else}_self{/if}" title="{$banner.button|escape:'htmlall':'UTF-8'}" class="button_large">{$banner.button|escape:'htmlall':'UTF-8'}</a>
                                    {/if}
                                    </div>
                                </div>
                            </div>
