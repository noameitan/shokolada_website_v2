...
{if isset($slide_group) && is_array($slide_group)}
    ...
    {foreach $slide_group as $slide}
        {if isset($slide['slide']) && is_array($slide['slide']) && count($slide['slide'])}
            ...
            <div class="slideshow_block small_slideshow {if $slide['hide_on_mobile']}hidden-phone{/if} mar_b1 clearfix">
            ...
            {else}
                <div class="block slideshow_block small_slideshow {if $slide['hide_on_mobile']}hidden-phone{/if}">
                    {assign var="banner" value=$slide['slide'][0]}
                    {if $banner.url}<a href="{$banner.url|escape:'html'}" target="{if $banner.new_window}_blank{else}_self{/if}">{/if}
                    <img src="{if (isset($banner.image_multi_lang) && $banner.image_multi_lang)}{$banner.image_multi_lang}{else}{$banner.image}{/if}" />
                    {if $banner.url}</a>{/if}
                </div>
            {/if}
        {/if}
