...
class StBlogClass extends ObjectModel
{
    ...
    public function getLinkProducts($associated = true, Context $context = null)
	{
		...
		SELECT DISTINCT pl.`id_product`, pl.`name`, pl.`link_rewrite`, p.`reference`, i.`id_image`, product_shop.`show_price`, cl.`link_rewrite` category, p.`ean13`
        ...
        foreach($id_products AS $k => $id_product)
        {
           ...
           if (!$exist)
                $ret &= Db::getInstance()->execute('
                    ...
                    `id_st_blog`, `id_product`, `active`, `position`)
                    VALUES('.(int)$this->id.','.(int)$id_product.',1,'.(int)$k.')
                '); 
        }
    ...
    public static function getRecentArticles($id_lang, $nbr_blogs = 10, Context $context = null)
    {
        Shop::addTableAssociation('st_blog', array('type' => 'shop'));
	...
	*/
	public function getPositionInCategory()
	{
		$result = Db::getInstance()->executeS('SELECT `position`
			FROM `'._DB_PREFIX_.'st_blog_category_blog`
			WHERE `id_st_blog_category` = '.(int)$this->id_st_blog_category_default.'
    ...
    public static function getBlogRss($base_url, $limit = 15, $id_blog = '')
    {
        ...
        foreach($rs AS $v)
        {
            ...
            $ret[] = array(
                'title' => $v['name'],
                'desc'  => $desc,
                'link'  => Tools::safeOutput(context::getContext()->link->getModuleLink('stblog','stblog',array('id_blog'=>$v['id_st_blog'],'rewrite'=>$v['link_rewrite']))),
                'pub'   => $v['date_upd']
            );
        }
        return $ret;
