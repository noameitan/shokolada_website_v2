...
class StBlogImageClass extends ObjectModel
{
	...
	public function __construct($type = null, $id = null, $id_lang = null)
	{
        ...
        if ($type && key_exists($type, self::$imageTypeDef))
	...
	{
        return self::$imageTypeDef[$this->image_type][$type];
	}
    ...
    {
        $ret = array();
        if (key_exists($this->image_type, self::$imageTypeDef))
        {
            $ret = self::$imageTypeDef[$this->image_type];
        }
        return $ret;
    }
    ...
    public function cropImages($tmp_file)
    {
       if (!is_file($tmp_file))
        ...
        foreach($this->getImageTypes() AS $key => $type)
        {
            ...
            $thumb = PhpThumbFactory::create($tmp_file_new);
            if (!$type[1])
                $thumb->adaptiveResizeWidth($width);
            else
                $thumb->adaptiveResize($width, $height);
            $thumb->save($folder.$file.$key.'.'.$ext, $ext);
            $ret &= ImageManagerCore::isRealImage($folder.$file.$key.'.'.$ext);
        }
     ...
     */
     ...
     public function getImageUrl($type = '')
     {
        ...
        return _PS_CURRENT_MODULE_IMG_DIR.$this->getImgFolder().$this->getImageName().$type.'.'.$this->image_format;
     }
    public static function getImageLinks($image,$type=null)
	{
	    if($type==null && $image)
            $type = $image['type'];
        if(!$type || !array_key_exists($type,self::$imageTypeDef))
            $type = 1;
	    ...
	    foreach(self::$imageTypeDef[$type] as $k=>$v)
        {
            if($use_default)
                $uri_path = $st_blog_folder.self::$imageTypeDefault[$type][$k];
            else
            {
    ...
    public static function getImageIdByType($id_st_blog = 0, $type = '1')
    {
        ...
        if (!key_exists($type, self::$imageTypeDef))
            return $ret;
