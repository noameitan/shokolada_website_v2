	...
	{foreach $blogs as $blog}
		<li class="block_blog {if $blog@first}first_item{elseif $blog@last}last_item{/if} clearfix">
            {if $blog.type==1 && $blog['covered']}
                <div class="blog_image"><a href="{$blog.link|escape:'html'}" rel="bookmark" title="{$blog.name|escape:'htmlall':'UTF-8'}"><img src="{$blog.cover.links.large}" alt="{$blog.name|escape:'htmlall':'UTF-8'}" width="{$imageSize[1]['large'][0]}" height="{$imageSize[1]['large'][1]}" class="hover_effect" /></a></div>
            {/if}
            ...
            {if $blog.type==2 && isset($blog['galleries']) && $blog['galleries']|count}
                ...
                <div class="blog_flexslider flexslider">
                    <ul class="slides">
                        ...
                        <li>
                          <a href="{$blog.link|escape:'html'}" rel="bookmark" title="{$blog.name|escape:'htmlall':'UTF-8'}"><img src="{$gallery.links.large}" alt="{$blog.name|escape:'htmlall':'UTF-8'}" width="{$imageSize[1]['large'][0]}" height="{$imageSize[1]['large'][1]}" class="hover_effect" /></a>
                        </li>
                        {/foreach}
                    </ul>
                </div>
            ...
            {elseif $blog.type==3 && $blog['covered']}
                <a href="{$blog.link|escape:'html'}" rel="bookmark" title="{$blog.name|escape:'htmlall':'UTF-8'}"><div class="blog_image"><img src="{$blog.cover.links.large}" alt="{$blog.name|escape:'htmlall':'UTF-8'}" width="{$imageSize[1]['large'][0]}" height="{$imageSize[1]['large'][1]}" class="hover_effect" /></div></a>
            {/if}
            <div>
