	...
	{if $blog->id AND $blog->active}
        <div id="blog_primary_block">
            {if $blog->type==1 && isset($cover)}
                <div class="blog_image"><img src="{$cover.links.large}" alt="{$blog->name|escape:'htmlall':'UTF-8'}" width="{$imageSize[1]['large'][0]}" height="{$imageSize[1]['large'][1]}" class="hover_effect" /></div>
            {/if}
            ...
            {if $blog->type==2 && isset($galleries) && $galleries|count}
                ...
                <div class="blog_flexslider flexslider">
                    <ul class="slides">
                        ...
                        <li>
                          <img src="{$gallery.links.large}" alt="{$blog->name|escape:'htmlall':'UTF-8'}" width="{$imageSize[1]['large'][0]}" height="{$imageSize[1]['large'][1]}" class="hover_effect" />
                        </li>
                        {/foreach}
                    </ul>
                </div>
        ...
        <section id="blog_related_products" class="block products_block section">
        	<h4 class="title_block"><span>{l s='Related prodcuts' mod='stblog'}</span></h4>
                	...
                	{foreach $related_products as $product}
                		<li class="ajax_block_product {if $product@first}first_item{elseif $product@last}last_item{else}item{/if}">
                            ...
                            <div class="pro_first_box">
                                <a href="{$link->getProductLink($product.id_product, $product.link_rewrite, $product.category)|escape:'html'}" title="{$product.name|escape:html:'UTF-8'}" class="product_image"><img src="{$link->getImageLink($product.link_rewrite,$product.id_product|cat:'-'|cat:$product.id_image,'home_default')}" height="{$homeSize.height}" width="{$homeSize.width}" alt="{$product.name|escape:html:'UTF-8'}"  />{$smarty.capture.new_on_sale}</a>
                            </div>
            ...
            jQuery(function($) {
                $('#blog_related-itemslider .sliderwrap').flexslider({
                    ...
                    itemWidth: 280,
                    minItems: getFlexSliderSize({/literal}{$items}{literal}),
                    maxItems: getFlexSliderSize({/literal}{$items}{literal}),
                ...
                $(window).resize(function(){
                    ...
                    blog_related_flexslider_rs = setTimeout(function() {
                        var flexSliderSize = getFlexSliderSize({/literal}{$items}{literal});
