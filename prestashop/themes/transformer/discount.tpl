...
<h1 class="heading">{l s='My vouchers'}</h1>

{if isset($cart_rules) && count($cart_rules) && $nb_cart_rules}
<table id="my_vouchers_list" class="discount std table_block">
	...
	{foreach from=$cart_rules item=discountDetail name=myLoop}
		<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
			<td class="discount_code">
                <div class="mobile_table_title visible-phone">{l s='Code'}</div>
			...
			<td class="discount_value">
                <div class="mobile_table_title visible-phone">{l s='Value'}</div>
			...
			<td class="discount_minimum">
                <div class="mobile_table_title visible-phone">{l s='Minimum'}</div>
                <div class="mobile_table_content">
			...
			<td class="discount_cumulative">
                <div class="mobile_table_title visible-phone">{l s='Cumulative'}</div>
                <div class="mobile_table_content">
			...
			<td class="discount_expiration_date">
                <div class="mobile_table_title visible-phone">{l s='Expiration date'}</div>
                <div class="mobile_table_content">{dateFormat date=$discountDetail.date_to}</div>
            </td>
		</tr>
	{/foreach}
	</tbody>
...
<ul class="footer_links clearfix">
	<li><a href="{$link->getPageLink('my-account', true)|escape:'html'}"><i class="icon-left icon-mar-lr2"></i>{l s='Back to Your Account'}</a></li>
	<li class="f_right"><a href="{$base_dir}"><i class="icon-home icon-mar-lr2"></i>{l s='Home'}</a></li>
</ul>