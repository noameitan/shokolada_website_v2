...
<h1 class="heading">{l s='Order history'}</h1>
		...
		{foreach from=$orders item=order name=myLoop}
			<tr class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{/if}">
				<td class="history_link bold">
            		<div class="mobile_table_title visible-phone">{l s='Order reference'}</div>
				...
				<td class="history_method">
            		<div class="mobile_table_title visible-phone">{l s='Payment'}</div>
                    <div class="mobile_table_content">
                    {$order.payment|escape:'htmlall':'UTF-8'}
                    </div>
                </td>
				<td class="history_state">
            		<div class="mobile_table_title visible-phone">{l s='Status'}</div>
                    <div class="mobile_table_content">
                    {if isset($order.order_state)}{$order.order_state|escape:'htmlall':'UTF-8'}{/if}
                    </div>
                </td>
				<td class="history_invoice">
            		<div class="mobile_table_title visible-phone">{l s='Invoice'}</div>
                    <div class="mobile_table_content">
				...
				<td class="history_detail">
            		<div class="mobile_table_title visible-phone">{l s=''}</div>
                    <div class="mobile_table_content">
