...
var ajaxCart = {
	...
	overrideButtonsInThePage : function(){
        ...
        if(!$('#add_to_cart').hasClass('quickview_addtocart'))
	...
	add : function(idProduct, idCombination, addedFromProductPage, callerElement, quantity, wishlist){
        ...
        if(st_addtocart_animation==0)    
        {
            art.dialog({
                id: 'add_to_cart_dialog',
                fixed: true,
                opacity: 0.5,
                lock: true
            });  
            $('#pro_added_continue').unbind('click').click(function(){
                art.dialog({id: 'add_to_cart_dialog'}).close();
            });
        }
		...
		$.ajax({
			...
			success: function(jsonData,textStatus,jqXHR)
			{
                ...
                {
                    var has_combination = (parseInt(idCombination) && idCombination != null);
                    $(jsonData.products).each(function(){
            			if (this.id == idProduct && (!has_combination || (has_combination && parseInt(idCombination) == this.idCombination )))
                        {
            			    if((st_addtocart_animation==1 || st_addtocart_animation==2) && ($("#header #cart_block").length || $(window).width()>768))
                            {
                                if(!callerElement && typeof(stquickview)!='undefined' && typeof(stquickview.callerElement)!='undefined' && stquickview.callerElement)
                                    callerElement = stquickview.callerElement;
                            ...
                            {
                                $('#pro_added_img').empty().append('<img src="' + this.image_link + '" alt="' + this.name + '" width="'+this.image_width+'" height="'+this.image_height+'" />');
                                $('#pro_added_title').empty().html(this.full_name);
                                $('#pro_added_cart_nbr').html(jsonData.nbTotalProducts);
                                $('#pro_added_cart_total').html(jsonData.total);
                                art.dialog({id: 'add_to_cart_dialog'}).content(document.getElementById('pro_added_wrap'));
                                ajaxCart.updateCartInformation(jsonData, addedFromProductPage);
                            } 
                            /*
                            if($('#rightbar_cart .rightbar_fly_num').size())
                                $('#rightbar_cart .rightbar_fly_num').html('+'+((quantity && quantity != null) ? quantity : '1')).css("top", 28).fadeIn().animate({
                            		"top": 0
                            	}, 800, function() {
                            		$(this).fadeOut();
                            	});  
			...
			for (i=0; i < jsonData.discounts.length; i++)
			{
				if (parseFloat(jsonData.discounts[i].price_float) > 0)
				{
					...
					if (jsonData.discounts[i].code.length)
						delete_link = '<a class="delete_voucher" href="'+jsonData.discounts[i].link+'" title="'+delete_txt+'"><i class="icon-trash"></i></a>';
		...
		$(jsonData.products).each(function(){
			...
			if (this.id != undefined)
			{
				...
				if ($('#cart_block_product_'+ domIdProduct).length == 0)
				{
					...
					var name = (this.name.length > 12 ? this.name.substring(0, (min>0 && ((min - 10) <= 7)) ? min : 10) + '...' : this.name);
					...
					if (this.hasAttributes)
						  content += '<div id="cart_block_combination_of_' + domIdProduct + '" class="hidden"><a href="' + this.link + '" title="' + this.name + '" class="color_666">' + this.attributes + '</a>';
	...
	$("#shopping_cart").hover(
		function() {
			if (ajaxCart.nb_total_products > 0 || cart_qty > 0)
				$("#header #cart_block").css('top',$(this).position().top+$(this).outerHeight(true)).stop(true, true).slideDown(450);
		},
		function() {
			setTimeout(function() {
