...
<h1 class="heading">{l s='Product Comparison'}</h1>
		...
		{foreach from=$products item=product name=for_products}
			...
			<td width="{$width}%" class="ajax_block_product comparison_infos">
				<a href="{$product->getLink()}" title="{$product->name|escape:html:'UTF-8'}" class="product_image" >
                    ...
                    {if isset($product->show_price) && $product->show_price && !isset($restricted_country_mode) && !$PS_CATALOG_MODE &&  $product->on_sale}<span class="on_sale"><i>{l s='Sale'}</i></span>{/if}
				</a>
				<h4 class="s_title_block"><a href="{$product->getLink()}" title="{$product->name|escape:'htmlall':'UTF-8'}">{$product->name|truncate:40:'...'|escape:'htmlall':'UTF-8'}</a></h4>
                ...
                <div class="clearfix">
				<a class="cmp_remove fr" href="{$link->getPageLink('products-comparison', true)|escape:'html'}" rel="ajax_id_product_{$product->id}"><i class="icon-trash icon-mar-lr2"></i>{l s='Remove'}</a>
				{if (!$product->hasAttributes() OR (isset($add_prod_display) AND ($add_prod_display == 1))) AND $product->minimal_quantity == 1 AND $product->customizable != 2 AND !$PS_CATALOG_MODE}
					{if ($product->quantity > 0 OR $product->allow_oosp)}
						<a class="exclusive ajax_add_to_cart_button fl" rel="ajax_id_product_{$product->id}" href="{$link->getPageLink('cart', true, NULL, "qty=1&amp;id_product={$product->id}&amp;token={$static_token}&amp;add")|escape:'html'}" title="{l s='Add to cart'}">{l s='Add to cart'}</a>
					{else}
                        <a class="button exclusive view_button fl" href="{$product->getLink()}" title="{l s='View'}">{l s='View'}</a>
					{/if}
				{else}
