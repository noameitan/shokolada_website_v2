...
<div id="wishlist_block" class="block account">
	...
	<div class="block_content">
		...
		{if $wishlist_products}
			...
			<dl id="wishlist_product_{$product.id_product}" class="products">
				<dt class="{if $smarty.foreach.i.first}first_item{elseif $smarty.foreach.i.last}last_item{else}item{/if}">
					...
					<a class="ajax_cart_block_remove_link" href="javascript:;" onclick="javascript:WishlistCart('wishlist_block_list', 'delete', '{$product.id_product}', {$product.id_product_attribute}, '0', '{if isset($token)}{$token}{/if}');" title="{l s='remove this product from my wishlist' mod='blockwishlist'}"><i class="icon-trash"></i></a>
				</dt>
		...
		<p id="wishlist_no_products" class="{if $wishlist_products && count($wishlist_products)}hidden{/if}">
		...
		{/if}
			<a href="{$wishlist_link}" title="{l s='My wishlists' mod='blockwishlist'}" class="go">{l s='My wishlists' mod='blockwishlist'}</a>
		</p>
	</div>
</div>
