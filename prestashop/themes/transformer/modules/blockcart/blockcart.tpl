...
<a href="{$link->getPageLink("$order_process", true)}" id="shopping_cart" title="{l s='Cart' mod='blockcart'}" rel="nofollow" class="clearfix">
<div class="ajax_cart_left icon_wrap">
<i class="icon-basket icon-0x"></i>
<span class="icon_text">{l s='Cart' mod='blockcart'}</span>
...
<div id="pro_added_wrap" class="dialog_message">
    ...
    <div id="pro_added_success" class="success">{l s='has been added to your cart.' mod='blockcart'}</div>
    <div class="pad_10">
        {l s='Your cart now have' mod='blockcart'} <span id="pro_added_cart_nbr"></span> {l s='item(s).' mod='blockcart'}<br />
        {l s='Total' mod='blockcart'}:<span id="pro_added_cart_total"></span><br />
    </div>
    <div class="dialog_action clearfix">
        <a id="pro_added_continue" class="fl button" href="javascript:;" rel="nofollow">{l s='Continue shopping' mod='blockcart'}</a>
        <a href="{$link->getPageLink("$order_process", true)}" id="button_order_cart" class="exclusive{if $order_process == 'order-opc'}_large{/if} btn fr" title="{l s='Check out' mod='blockcart'}" rel="nofollow"><span></span>{l s='Check out' mod='blockcart'}</a>
    </div>
</div>
...
<div id="cart_block" class="block exclusive">
	...
	{if $products}
		...
		{foreach from=$products item='product' name='myLoop'}
			...
			<div id="cart_block_combination_of_{$product.id_product}{if $product.id_product_attribute}_{$product.id_product_attribute}{/if}_{$product.id_address_delivery|intval}" class="{if $smarty.foreach.myLoop.first}first_item{elseif $smarty.foreach.myLoop.last}last_item{else}item{/if}">
				<a href="{$link->getProductLink($product, $product.link_rewrite, $product.category, null, null, $product.id_shop, $product.id_product_attribute)|escape:'html'}" title="{l s='Product detail' mod='blockcart'}" class="color_666">{$product.attributes_small}</a>
			{/if}
			<!-- Customizable datas -->
	...
	{/if}
		...
		<div id="cart-prices">
			<div class="cart-prices-item clearfix">
			    ...
			    <span class="fl">{l s='Shipping' mod='blockcart'}</span>
            </div>
			{if $show_wrapping}
				...
				<span class="fl">{l s='Wrapping' mod='blockcart'}</span>
                </div>
			{/if}
			{if $show_tax && isset($tax_cost)}
                <div class="cart-prices-item clearfix">
				<span id="cart_block_tax_cost" class="price ajax_cart_tax_cost">{$tax_cost}</span>
				<span class="fl">{l s='Tax' mod='blockcart'}</span>
				</div>
			{/if}
			<div class="cart-prices-item clearfix">
		...
		<div id="cart-buttons" class="clearfix">
			{if $order_process == 'order'}<a href="{$link->getPageLink("$order_process", true)|escape:'html'}" class="button_small btn fl" title="{l s='View my shopping cart' mod='blockcart'}" rel="nofollow">{l s='Cart' mod='blockcart'}</a>{/if}
			<a href="{$link->getPageLink("$order_process", true)|escape:'html'}" id="button_order_cart" class="exclusive{if $order_process == 'order-opc'}_large{/if} btn fr" title="{l s='Check out' mod='blockcart'}" rel="nofollow"><span></span>{l s='Check out' mod='blockcart'}</a>
		</div>
	</div>
	</div>
</div>
