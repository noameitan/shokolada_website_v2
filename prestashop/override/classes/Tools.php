	...
	public static function getPath($id_category, $path = '', $link_on_the_item = false, $category_type = 'products', Context $context = null)
	{
		...
		if ($category_type === 'products')
		{
			...
			if ($interval)
			{
				...
				{
					$full_path .=
					(($n < $n_categories || $link_on_the_item) ? '<li typeof="v:Breadcrumb"><a rel="v:url" property="v:title" href="'.Tools::safeOutput($context->link->getCategoryLink((int)$category['id_category'], $category['link_rewrite'])).'" title="'.htmlentities($category['name'], ENT_NOQUOTES, 'UTF-8').'">' : '<li typeof="v:Breadcrumb"><span property="v:title">').
